...
class network:
    ...
    def build_network(self):
        ...
        self.input=tf.placeholder(tf.float32,shape=[None,270,480,3])
        ...
        output_11 = tf.nn.leaky_relu(conv2d(output_7, W_conv11, stride=1) + b_conv11, alpha=0.1)
        # 最终维度：17x30x512
        ...
        output_15 = tf.nn.leaky_relu(conv2d(output_12, W_conv15, stride=1) + b_conv15, alpha=0.1)
        # 最终维度：17x30x512
        ...
        output_19 = tf.nn.leaky_relu(conv2d(output_17, W_conv19, stride=1) + b_conv19, alpha=0.1)
        # 最终维度：9x15x1024
        ...
        #最终维度：5x8x1024
        W_fc1=weight_variable([5*8*1024,2])
        b_fc1=bias_variable([2])
        fc1_input=tf.reshape(aver_output,[-1,5*8*1024])
    ...
    def train(self):
        # load the tfrecord
        image,label=Cl.read_and_decode('catch_classification_train.tfrecords',[270,480,3])
